...
namespace MkvDefaultTrackChanger.GtkSharp
{
    public class CustomFilePickerHandler : GtkControl<Gtk.EventBox, FilePicker, FilePicker.ICallback>, FilePicker.IHandler
	{
		...
		public CustomFilePickerHandler()
		{
			...
			// START new code
			filesBox = new Gtk.HBox();
			selectFilesButton = new Gtk.Button();
			selectFilesButton.Label = "Browse";
			filesBox.PackStart(selectFilesButton, false, true, 1);

			Control.Child = filesBox;
		}
		...
		protected override void Initialize()
		{
			...
			selectFilesButton.Clicked += MultipleFileSelection_Changes;
		...
		public void MultipleFileSelection_Changes(object sender, EventArgs e)
        {
			var filedialog = new Gtk.FileChooserDialog("Choose files", null, Gtk.FileChooserAction.Open,
				"Select", Gtk.ResponseType.Ok, "Cancel", Gtk.ResponseType.Close);
			filedialog.SelectMultiple = true;

			filedialog.Title = filebutton.Title;
			foreach (var filter in filebutton.Filters)
				filedialog.AddFilter(filter);
			filedialog.Filter = filebutton.Filter;

			var result = filedialog.Run();
			filedialog.Hide();
			filedialog.Unrealize();

			if (result == (int)Gtk.ResponseType.Ok)
            {
				this.FilePath = String.Join('|', filedialog.Filenames);
			}
		...
		public FileAction FileAction
		{
			...
			set
			{
				...
				else
				{
					...
					Control.Child = selectFilesButton;
				}
